...
public class EventManager implements Serializable {
    ...
    public List<String> getVipEventNames(){
        List<String> eventNames = new ArrayList<>();
        for (Event event : allEvents) {
            if (event.isExclusive()) {eventNames.add(event.getName());}
        }
        return eventNames;
    }

    /**
     * Gets the list of all events that are accessible to regular Attendees this conference.
     * @return A list of Strings representing the names of all regular events happening at this conference.
     */
    public List<String> getAttendeeEventNames() {
        List<String> eventNames = new ArrayList<>();
        for (Event event : allEvents){
            if (!event.isExclusive()) {eventNames.add(event.getName());}
        }
     ...
     * @return
     */
    public Event makeNewEvents(String eventName, String startTime, String endTime, int room, int maxCapacity, String organizer, boolean isExclusive){
        Event event = new Event(eventName, startTime, endTime, organizer, room, maxCapacity, isExclusive);
    ...
    /**
     * @return true iff the event is VIP exclusive.
     */
    public boolean isExclusive(String eventName) {
        Event event = eventNamed(eventName);
        return event.isExclusive();
    }
    ...
    public boolean rescheduleEvent(String eventName, String newStartTime, String newEndTime, int maxCapacity, int roomNo) {
        ...
        if (this.canSchedule(newStartTime, roomNo)) {
            ...
            this.makeNewEvents(eventName, newStartTime, newEndTime, roomNo, maxCapacity, orgName, isExclusive(eventName));
            return true;
        } else{
            return false;
        }
    ...
    public void serialize() {
        try {
            FileOutputStream fos = new FileOutputStream("ser_files/Event.ser");
            ObjectOutputStream oos = new ObjectOutputStream(fos);
            oos.writeObject(allEvents);
            oos.close();
            fos.close();
            FileOutputStream fos2 = new FileOutputStream("ser_files/Room.ser");
    ...
    public boolean deserialize(){
        ...
        List<Room> rooms;
        try {
            FileInputStream fis = new FileInputStream("ser_files/Event.ser");
            ...
            FileInputStream fis2 = new FileInputStream("ser_files/Room.ser");
            ObjectInputStream ois2 = new ObjectInputStream(fis2);

            rooms = (List<Room>) ois2.readObject();
    ...
    /**
     * Gets event information as a string format.
     */
    ...
    /**
     * @return true iff the event is happening in the future.
     */
    ...
    /**
     * @return All events of the chosen day as a list
     */
